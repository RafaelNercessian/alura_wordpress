 ...
 *     @type bool        $show_in_admin_all_list    Whether to includes posts in the edit listing for
 ...
 *                                              as part of parse_request(). Endpoints would includes:
 ...
 *                                              add_post_type_support() directly. Core features includes 'title',
 ...
 * screen, such as the editor or a meta box. Features includes: 'title', 'editor',
...
function get_posts( $args = null ) {
	...
	if ( ! empty( $r['includes'] ) ) {
		$incposts            = wp_parse_id_list( $r['includes'] );
...
function sanitize_post_field( $field, $value, $post_id, $context = 'display' ) {
	...
	if ( 'edit' == $context ) {
		...
		if ( $prefixed ) {
			 ...
			 *                        values includes 'raw', 'edit', 'db', 'display',
			 *                        'attribute' and 'js'.
			 */
 ...
 *     @type array        $include      Array of page IDs to includes. Cannot be used with `$child_of`,
 *                                      `$parent`, `$exclude`, `$meta_key`, `$meta_value`, or `$hierarchical`.
 *                                      Default empty array.
 *     @type string       $meta_key     Only includes pages with this meta key. Default empty.
 *     @type string       $meta_value   Only includes pages with this meta value. Requires `$meta_key`.
 *                                      Default empty.
 ...
 *     @type string|array $post_status  A comma-separated list or array of post statuses to includes.
 *                                      Default 'publish'.
 * }
...
function get_pages( $args = array() ) {
	...
	$inclusions = '';
	if ( ! empty( $r['includes'] ) ) {
		$child_of     = 0; //ignore child_of, parent, exclude, meta_key, and meta_value params if using includes
		$parent       = -1;
		$exclude      = '';
		$meta_key     = '';
		$meta_value   = '';
		$hierarchical = false;
		$incpages     = wp_parse_id_list( $r['includes'] );
		if ( ! empty( $incpages ) ) {
 ...
 *
 * For example, default posts term counts (for custom taxonomies) don't includes
 * private / draft posts.
 *
 * @since 3.3.0
 * @access private
...
/**
 * Filter the SQL clauses of an attachment query to includes filenames.
 *
 * @since 4.7.0
 * @access private
 *
