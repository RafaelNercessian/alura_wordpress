...
var describeComponentFrame = function (name, source, ownerName) {
  ...
  if (source) {
    ...
    {
      // In DEV, includes code for a common special case:
...
function updateNamedCousins(rootNode, props) {
  ...
  if (props.type === 'radio' && name != null) {
    ...
    // and won't includes inputs that use the HTML5 `form=` attribute. Since
...
// so we will keep track of it, and includes it in the report.
...
//     Base state: 'A'            <-  The base state does not includes C1,
...
function logCapturedError(capturedError) {
  ...
  var error = capturedError.error;
  {
    ...
    // We don't includes the original error message and JS stack because the browser
