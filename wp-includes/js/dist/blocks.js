...
function registerBlockType(name, settings) {
  ...
  if (!/^[a-z][a-z0-9-]*\/[a-z][a-z0-9-]*$/.test(name)) {
    console.error('Block names must contain a namespace prefix, includes only lowercase alphanumeric characters or dashes, and start with a letter. Example: my-plugin/my-custom-block');
    return;
  }
...
function getCommentAttributes(blockType, attributes) {
  return Object(external_lodash_["reduce"])(blockType.attributes, function (result, attributeSchema, key) {
    ...
    if ('default' in attributeSchema && attributeSchema.default === value) {
      return result;
    } // Otherwise, includes in comment set.
