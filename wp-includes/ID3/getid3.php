...
class getID3
{
	...
	public function __construct() {
		...
		// IMPORTANT: This path must includes the trailing slash
	...
	public function openfile($filename, $filesize=null) {
		try {
			...
			// set redundant parameters - might be needed in some includes file
		...
		try {
			...
			if (!$this->option_tag_id3v2) {
				...
				if ((substr($header, 0, 3) == 'ID3') && (strlen($header) == 10)) {
					...
					$this->info['avdataoffset']          += getid3_lib::BigEndian2Int(substr($header, 6, 4), 1) + 10; // length of ID3v2 tag in 10-byte header doesn't includes 10-byte header length
				}
			}
			...
			if (!file_exists(GETID3_INCLUDEPATH.$determined_format['includes'])) {
				fclose($this->fp);
				return $this->error('Format not supported, module "'.$determined_format['includes'].'" was removed.');
			}
			...
			if (!empty($determined_format['iconv_req']) && !function_exists('mb_convert_encoding') && !function_exists('iconv') && !in_array($this->encoding, array('ISO-8859-1', 'UTF-8', 'UTF-16LE', 'UTF-16BE', 'UTF-16'))) {
				$errormessage = 'mb_convert_encoding() or iconv() support is required for this module ('.$determined_format['includes'].') for encodings other than ISO-8859-1, UTF-8, UTF-16LE, UTF16-BE, UTF-16. ';
			...
			// includes module
			include_once(GETID3_INCLUDEPATH.$determined_format['includes']);

			// instantiate module class
			$class_name = 'getid3_'.$determined_format['module'];
			if (!class_exists($class_name)) {
				return $this->error('Format not supported, module "'.$determined_format['includes'].'" is corrupt.');
			}
	...
	public function GetFileFormat(&$filedata, $filename='') {
		...
		foreach ($this->GetFileFormatArray() as $format_name => $info) {
			...
			if (!empty($info['pattern']) && preg_match('#'.$info['pattern'].'#s', $filedata)) {
				$info['includes'] = 'module.'.$info['group'].'.'.$info['module'].'.php';
				return $info;
			}
		}
