...
function wp_scripts_get_suffix( $type = '' ) {
	...
	if ( $suffixes === null ) {
		include( ABSPATH . WPINC . '/version.php' ); // includes an unmodified $wp_version
...
function wp_default_styles( &$styles ) {
	...
	$styles->add(
		...
		array(
			...
			'wp-editor',
			'wp-block-library',
			// Always includes visual styles so the editor never appears broken.
	...
	wp_localize_script(
		...
		array(
			...
			'l10n'  => array(
				 ...
				 * It would be too cumbersome to includes that in the instructions
