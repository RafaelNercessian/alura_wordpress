...
final class _WP_Editors {
	 ...
	 *                                         Should includes `<style>` tags, and can use "scoped". Default empty.
	 *     @type string     $editor_class      Extra classes to add to the editor textarea element. Default empty.
	 *     @type bool       $teeny             Whether to output the minimal editor config. Examples includes
		...
		if ( self::$this_tinymce ) {
			...
			if ( empty( self::$first_init ) ) {
				...
				if ( $set['teeny'] ) {
					 ...
					 *
					 * The url should be absolute, and should includes the js filename
					...
					if ( ! empty( $mce_external_plugins ) ) {
						 ...
						 * where 'path' is the includes path to the file.
						 *
	 ...
	 * Doesn't includes plugins, buttons, editor selector.
	 *
	 * @global string $tinymce_version
	 *
	 * @return array
	 ...
	 * @param bool $json_only optional Whether to includes the JavaScript calls to tinymce.addI18n() and tinymce.ScriptLoader.markDone().
