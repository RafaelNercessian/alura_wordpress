...
class WP_REST_Posts_Controller extends WP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		// Ensure an includes parameter is set in case the orderby is set to 'includes'.
		if ( ! empty( $request['orderby'] ) && 'includes' === $request['orderby'] && empty( $request['includes'] ) ) {
			return new WP_Error( 'rest_orderby_include_missing_include', __( 'You need to define an includes parameter to order by includes.' ), array( 'status' => 400 ) );
		}
