...
class WP_User_Query {
	 ...
	 *
	 * @since 3.1.0
	 * @since 4.1.0 Added the ability to order by the `includes` value.
	 ...
	 *     @type array        $includes             An array of user IDs to includes. Default empty array.
	 ...
	 *                                             'ID', 'display_name' (or 'name'), 'includes', 'user_login'
	 ...
	 *                                             Use 'all' for all fields and 'all_with_meta' to includes
	 ...
	 *     @type array        $nicename__in        An array of nicenames to includes. Users matching one of these
	 ...
	 *     @type array        $login__in           An array of logins to includes. Users matching one of these
	...
	public function prepare_query( $query = array() ) {
		...
		// Parse and sanitize 'includes', for use by 'orderby' as well as 'includes' below.
		if ( ! empty( $qv['includes'] ) ) {
			$include = wp_parse_id_list( $qv['includes'] );
		} else {
			$include = false;
		}
		...
		if ( $search ) {
			 ...
			 *
			 * The default columns depend on the search term, and includes 'user_email',
	...
	protected function parse_orderby( $orderby ) {
		...
		} elseif ( 'includes' === $orderby && ! empty( $this->query_vars['includes'] ) ) {
			$include     = wp_parse_id_list( $this->query_vars['includes'] );
