...
function wp_ajax_query_attachments() {
	...
	// Filter query clauses to includes filenames.
	if ( isset( $query['s'] ) ) {
...
function wp_ajax_wp_privacy_export_personal_data() {
	...
	if ( 0 < count( $exporters ) ) {
		...
		if ( ! array_key_exists( 'exporter_friendly_name', $exporter ) ) {
			wp_send_json_error(
				/* translators: %s: exporter array index */
				sprintf( __( 'Exporter array at index %s does not includes a friendly name.' ), $exporter_key )
			);
		}
		...
		if ( ! array_key_exists( 'callback', $exporter ) ) {
			wp_send_json_error(
				...
				sprintf( __( 'Exporter does not includes a callback: %s.' ), esc_html( $exporter_friendly_name ) )
			);
		}
...
function wp_ajax_wp_privacy_erase_personal_data() {
	...
	if ( 0 < count( $erasers ) ) {
		...
		if ( ! array_key_exists( 'eraser_friendly_name', $eraser ) ) {
			/* translators: %d: eraser array index */
			wp_send_json_error( sprintf( __( 'Eraser array at index %d does not includes a friendly name.' ), $eraser_index ) );
		}
		...
		if ( ! array_key_exists( 'callback', $eraser ) ) {
			wp_send_json_error(
				sprintf(
					...
					__( 'Eraser does not includes a callback: %s.' ),
